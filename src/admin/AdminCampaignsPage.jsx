...
function Row({ c, onDone }) {
  ...
  const onRecover = async () => {
    ...
    try {
      ...
      if (r?.result) setMsg(`Local:${r.result.local} News:${r.result.ingest} Analyses:${r.result.analyses}`);
  ...
  return (
    <tr className="border-b">
      <td className="py-2 pr-3"><Link className="underline" to={`/admin/campaigns/${c.id}`}>{c.name}</Link></td>
      ...
      <td className="py-2">
        <button onClick={onRecover} disabled={loading} className="px-3 py-1.5 rounded-lg bg-black text-white text-sm disabled:opacity-60">
...
export default function AdminCampaignsPage() {
  ...
  const filtered = !q ? items : items.filter((c) => `${c.name} ${c.query}`.toLowerCase().includes(q.toLowerCase()));
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              filtered.map((c) => <Row key={c.id} c={c} onDone={load} />)
            )}
          </tbody>
        </table>
      </div>
